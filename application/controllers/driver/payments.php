    ...
    **/
	public function payment_summary($invoice_id=""){
		...
		if($billDetails->num_rows()>0){
			$fromdate=$billDetails->row()->bill_from->sec;
			$todate=$billDetails->row()->bill_to->sec;
			...
			$billingsList = $this->driver_model->get_all_details(TRANSACTION,array('bill_period_from'=>new \MongoDate($fromdate),'bill_period_to'=>new \MongoDate($todate)));
