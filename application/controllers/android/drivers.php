            ...
            if ($email != '' && $password != '') {
                if (valid_email($email)) {
                    ...
                    if ($checkDriver->num_rows() == 1) {
                        ...
                        $driverVal = $this->driver_model->get_selected_fields(DRIVERS, array('_id' => new \MongoId($checkDriver->row()->_id)), array('email', 'image', 'driver_name', 'push_notification', 'vehicle_number', 'vehicle_model', 'password'));
                ...
                if ($checkDriver->num_rows() == 1) {
                    ...
                    if ($checkRide->num_rows() == 1) {
                        if ($checkRide->row()->ride_status != 'Cancelled' || $checkRide->row()->ride_status != 'Arrived') {
                            ...
                            if (isset($userVal->row()->push_type)) {
                                if ($userVal->row()->push_type != '') {
                                    $message = 'Driver arrived on your place';
							...
							if(!empty($mapValues)){
								...
								if(($ride_begin_time+$ride_wait_time)<=$ride_end_time){
									...
									$summary = array('ride_distance' => round($distance, 2),
										'ride_duration' => round($ride_time_min, 2),
										'waiting_duration' => round($wait_time, 2)
									);
									...
									$firstRide = $this->driver_model->get_selected_fields(RIDES, array('user.id' => new \MongoId($checkRide->row()->user['id']), 'ride_status' => (string)$ride_status), array('_id'));
									if ($firstRide->num_rows() == 1) {
										...
										if ($get_referVal->num_rows() > 0) {
											...
											$this->driver_model->update_details(REFER_HISTORY, array('used' => 'true'), $condition);
									...
									$returnArr['response'] = $this->format_string("Enterd inputs are incorrect", "invalid_trip_end_inputs");
								}
                ...
                if ($driverVal->num_rows() > 0) {
                    ...
                    if ($checkRide->num_rows() == 1) {
                        ...
                        $responseArr = array('currency' => $checkRide->row()->currency,
                            ...
                            'drop' => $dropArr,
                            'pickup_date' => date("H:i A", $checkRide->row()->booking_information['booking_date']->sec) . ' on ' . date("jS M, Y", $checkRide->row()->booking_information['booking_date']->sec),
                            'summary' => $summaryArr,
                            'fare' => $fareArr,
                            'tips' => $tipsArr,
