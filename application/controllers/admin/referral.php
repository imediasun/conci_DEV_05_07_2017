...
class Referral extends MY_Controller {
    ...
    public function index() {
        if ($this->checkLogin('A') == '') {
            ...
            redirect('admin/referral/display_user_referrals');
        }
    }
    ...
    public function display_user_referrals() {
        if ($this->checkLogin('A') == '') {
            ...
            if ($usersCount > 1000) {
                ...
                $searchbaseUrl = 'admin/referral/display_user_referrals/';
                $config['num_links'] = 3;
                ...
                $this->load->view('admin/referral/display_userlist', $this->data);
            }
        }
    }
	...
	public function display_driver_referrals() {
        ...
        if ($driversCount > 1000) {
            ...
            $searchbaseUrl = 'admin/referral/display_driver_referrals/';
            $config['num_links'] = 3;
            $config['display_pages'] = TRUE;
            $config['base_url'] = $searchbaseUrl;
        ...
        $this->load->view('admin/referral/display_drivers_list', $this->data);
    }
	...
	public function view_referral_details(){
		...
		if($refType == 'user'){
			$condition = array('user_id' => new MongoId($mem_id));
			$getMember = $this->user_model->get_selected_fields(USERS,array('_id' => new MongoId($mem_id)),array('user_name','email','unique_code'));
			$this->data['heading'] = $getMember->row()->user_name.' ( '.$getMember->row()->email.' ) - '.$referred_members_list;
		} else {
			$condition = array('driver_id' => new MongoId($mem_id));
			$getMember = $this->user_model->get_selected_fields(DRIVERS,array('_id' => new MongoId($mem_id)),array('driver_name','email','unique_code'));
		...
		#echo '<pre>'; print_r($referralsList->row()->history); die;
		$this->load->view('admin/referral/view_referrals_list', $this->data);
	}
}
