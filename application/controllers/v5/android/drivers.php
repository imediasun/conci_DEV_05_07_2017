...
class Drivers extends MY_Controller {
    ...
    function __construct() {
		...
		if(stripos($auth_key,APP_NAME) === false) {
			...
			$apply_function = array('login_driver','update_driver_location');
							...
							if(!empty($mapValues)){
								...
								if(($ride_begin_time+$ride_wait_time)<=($ride_end_time+100)){
									...
									$fare_details = array('currency' => $currency,
										...
										'ride_duration' => round(ceil($ride_time_min), 2) . ' ' . $ride_time_unit,
									...
									$returnArr['response'] = array('need_payment' => $need_payment, 
																...
																'message' => $this->format_string('Ride Completed', 'ride_completed'),
																'ratting_content' => (string)$ratting_content,
	...
	**/
		...
		$checkLocation = $this->location_model->get_selected_fields(LOCATIONS, array('_id' => new \MongoId($checkDriver->row()->driver_location)),array('currency'));
		 $finalArr['currency']=$checkLocation->row()->currency;
            ...
            if ($checkRide->num_rows() > 0) {
                ...
                $returnArr['status'] = '1';
				$finalArr=$this->customMultiSort($finalArr,'begin_ride_time');
				$finalArr['trips'] = array_reverse($finalArr['trips']);
    ...
    public function provider_today_jobs()
    {
		...
		public function ratings_customMultiSort_with_subfield($array,$field,$subfield) {
		$sortArr = array();
		foreach($array as $key=>$val){
			$sortArr[$key] = $val['ratings'][$field][$subfield];
		}
        ...
        if ($checkDriver->num_rows() > 0) {
           ...
           if($get_ratings->num_rows() > 0){
               ...
               $num_reviews=$get_ratings->num_rows();
					...
					$final_ratingArr=$this->ratings_customMultiSort_with_subfield($reviewsList,'driver','time');
					$reviewsList=$final_ratingArr;
