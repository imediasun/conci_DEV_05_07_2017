...
class User extends MY_Controller {
    ...
    function __construct() {
		...
		if(stripos($auth_key,APP_NAME) === false) {
			...
			$apply_function = array('get_ride_status','if_user_onride','check_account','check_social_login','register_user','social_Login','login_user','proceed_payment','edit_user_image','edit_user_data','booking_ride','all_ride_list','get_drivers_in_map');
...
public function booking_ride() {
                       ...
                       $this->push_data=array();
                        ...
                        if (isset($checkUser->row()->user_name)) {
                            if ($checkUser->row()->user_name != '') {
                                $userArr['user_name'] = $checkUser->row()->user_name;
                            }
                        }
                        ...
                        if ($checkUser->row()->push_type != '') {
                            ...
                            if (!empty($location['result'])) {
                                ...
                                if ($categoryResult->num_rows() > 0) {
                                    ...
                                    $returnArr['status'] = '1';
                                    $returnArr['response'] = array('user'=>$userArr,'card_rate_per_minute'=>$locationReturn,'type' => (string) $type, 'response_time' => (string) $response_time + 10, 'ride_id' => (string) $ride_id, 'message' => $this->format_string('Booking Request Sent', 'booking_request_sent'), 'rider_location' => $riderlocArr);
                                } else {
                                    $returnArr['response'] = $this->format_string('No cabs available nearby', 'cabs_not_available_nearby');
                                }
                            } else {
	   ...
	   public function edit_user_data()
    {
	   ...
	   public function edit_user_image()
    {
                ...
                if (!empty($user_id)) {
                    ...
                    if ($user_details->num_rows() > 0) {
						if($image_status==1){
                                ...
                                if ($this->user_model->update_details(USERS,array('image'=>$file), array('_id' => new MongoId($user_id)))) {
                                    $returnArr['status'] = '1';
                                }
	   ...
	   public function get_ride_status()
    {
        ...
        try {
            ...
            if (isset($ride_id) ) {
                if (!empty($ride_id)) {
                    $ride_details = $this->app_model->get_selected_fields(RIDES, array('ride_id' => $ride_id), array('ride_status','booking_information.booking_date'));
                    
					if ($ride_details->num_rows() > 0) {
                        ...
                        if($ride_details->num_rows() > 0){
                            ...
                            $returnArr['response'] = array('ride_status' => $ride_details->row()->ride_status , 'booking_date'=>$ride_details->row()->booking_information['booking_date'] 
                            );
                        }
                        else {
                            $returnArr['response'] = $this->format_string("We have no ride details", "invalid_user");
                        }
                        ...
                        $returnArr['response'] = $this->format_string("Invalid Ride", "invalid_user");
                    }
                } else {
                    $returnArr['response'] = $this->format_string("Some Parameters are missing", "some_parameters_missing");
                }
