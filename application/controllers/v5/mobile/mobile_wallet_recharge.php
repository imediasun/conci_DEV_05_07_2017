...
class Mobile_wallet_recharge extends MY_Controller {
    ...
    public function add_pay_wallet_payment_form() {
        ...
        if ($this->data['auto_charge'] == 'Yes') {
			...
			$this->load->view('v5/mobile/wallet_recharge/stripe_payment_card.php', $this->mobdata);
        } else {
            $this->load->view('v5/mobile/wallet_recharge/wallet_payment_option.php', $this->mobdata);
        }
    }
    ...
    public function user_wal_PaymentCard() {
        ...
        if ($transaction_id != '' && $total_amount != '' && $user_id != '') {
            ...
            if ($response->approved) {
                redirect('v5/wallet-recharge/success/' . $user_id . '/' . $transaction_id . '/credit-card/' . $pay_txn_id);
            } else {
                redirect('v5/wallet-recharge/failed/Error ' . $response->response_reason_text . '?mobileId=' . $transaction_id);
            }
        } else {
            redirect('v5/wallet-recharge/pay-cancel?mobileId=' . $transaction_id);
        }
    }
    ...
    public function paypal_wal_payment_process() {
        ...
        if ($transaction_id != '' && $total_amount != '' && $user_id != '') {
            ...
            $this->paypal_class->add_field('return', base_url() . 'v5/wallet-recharge/success/' . $user_id . '/' . $transaction_id . '/paypal'); // Return URL

            $this->paypal_class->add_field('cancel_return', base_url() . 'v5/wallet-recharge/failed/cancelled'); // Cancel URL
        ...
        if ($user_id != '' && $cardInfo == 'Yes') {
            ...
            try {
                ...
                if ($auto_pay_status == 'Yes') {
                    ...
                    redirect(base_url() . 'v5/wallet-recharge/success/' . $user_id . '/' . $transaction_id . '/stripe?stripeTxnId=' . $charge['id']);
                }
            } catch (Exception $e) {
                $error = $e->getMessage();
                ...
                if ($auto_pay_status == 'Yes') {
                    ...
                    redirect(base_url() . 'v5/wallet-recharge/failed/' . $error . '?user_id=' . $user_id);
                }
            }
		} else {
			if($auto_pay_status == 'Yes'){
				...
				redirect(base_url() . 'v5/wallet-recharge/failed/Invalid Payment Request?user_id=' . $user_id);
			}
        }
    }
    ...
    public function pay_success($payMode = 'Manual', $paymentData = array()) {
        ...
        if ($payment_status == 'Completed') {
            ...
            if ($payMode == 'Auto') {
                ...
                redirect('v5/wallet-recharge/failed/Error');
            }
        }
    }
