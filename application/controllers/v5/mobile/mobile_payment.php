...
class Mobile_payment extends MY_Controller {
    ...
    function __construct() {
        ...
        if (isset($_GET['mobileId']) && isset($_POST['mobileId'])) {
            $this->load->view('v5/mobile/error.php', $this->mobdata);
            die;
        } else {
    ...
    public function credit_card_form() {
        $this->load->view('v5/mobile/credit_card_payment.php', $this->mobdata);
    }
    ...
    public function userPaymentCard() {
        ...
        if ($checkRide->num_rows() == 1) {
            ...
            if ($response->approved) {
                redirect('v5/mobile/success/' . $user_id . '/' . $ride_id . '/credit-card/' . $response->transaction_id . '?mobileId=' . $this->mobdata['mobileId']);
            } else {
                redirect('v5/mobile/failed/' . $response->response_reason_text . '?mobileId=' . $this->mobdata['mobileId']);
            }
        } else {
            redirect('v5/mobile/payment/Cancel?mobileId=' . $this->mobdata['mobileId']);
        }
    }
    ...
    public function paypal_payment_process() {
        ...
        if ($checkRide->num_rows() == 1) {
            ...
            $this->paypal_class->add_field('return', base_url() . 'v5/mobile/success/' . $user_id . '/' . $ride_id . '/paypal/?mobileId=' . $this->mobdata['mobileId']); // Return URL

            $this->paypal_class->add_field('cancel_return', base_url() . 'v5/mobile/failed/cancelled?mobileId=' . $this->mobdata['mobileId']); // Cancel URL

            $this->paypal_class->add_field('notify_url', base_url() . 'v5/paypal-payment-ipn'); // Notify url
    ...
    public function stripe_payment_form() {
        ...
        #$this->load->view('v4/mobile/stripe_payment.php', $this->mobdata);
		$this->load->view('v5/mobile/stripe_payment_card.php', $this->mobdata);
    }
    ...
    public function stripe_payment_process() {
        ...
        if ($checkRide->num_rows() == 1 && $cardInfo == 'Yes') {
            ...
            try {
                ...
                redirect('v5/mobile/success/' . $user_id . '/' . $ride_id . '/stripe/' . $charge['id'] . '?mobileId=' . $this->mobdata['mobileId']);
            } catch (Exception $e) {
                $error = $e->getMessage();
                if ($error == '') {
                ...
                redirect('v5/mobile/failed/' . $error . '?mobileId=' . $this->mobdata['mobileId']);
            }
        } else {
            redirect('v5/mobile/payment/Cancel?mobileId=' . $this->mobdata['mobileId']);
        }
    ...
    public function pay_success() {
        ...
        if ($payment_status == 'Completed') {
            ...
            $this->load->view('v5/mobile/success.php', $this->mobdata);
        } else {
            redirect('v5/mobile/failed/paymentfailed?mobileId=' . $this->mobdata['mobileId']);
        }
    }
    ...
    public function pay_failed() {
        ...
        $this->load->view('v5/mobile/failed.php', $this->mobdata);
    }
    ...
    public function payment_return() {
        ...
        $this->clearPayData();
        $this->load->view('v5/mobile/payment_return.php', $this->mobdata);
    }
