...
class Billing extends MY_Controller {
	...
	public function generate_billing(){
		...
		/*  #Testing Lines 
		$billingInfo = array('bill_generated'=> new \MongoDate(strtotime(date("Y-m-d 00:00:00"))));
		$this->billing_model->commonDelete(TRANSACTION,$billingInfo);
		$billingInfoz = array('bill_date'=> new \MongoDate(strtotime(date("Y-m-d 00:00:00"))));
		...
		if($hasBIlling=== TRUE){		
			...
			if($driverDetails->num_rows()>0){
				...
				$billingInfo = array('bill_id'=> (string)$bill_id,
												'bill_period_from'=> new \MongoDate($fdate),
												'bill_period_to'=> new \MongoDate($tdate),
												'bill_generated'=> new \MongoDate(strtotime(date("Y-m-d 00:00:00"))),
				...
				foreach ($driverDetails->result() as $driver){
					...
					$invoiceDetails = array('bill_id'=> (string)$bill_id,
						'bill_date'=> new \MongoDate(strtotime(date("Y-m-d 00:00:00"))),
						'invoice_id'=> $invoice_id,
						'bill_from'=> new \MongoDate(strtotime($billing_from)),
						'bill_to'=> new \MongoDate(strtotime($billing_to)),
						'driver_id'=> $driver_id,
