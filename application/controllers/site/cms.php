...
class Cms extends MY_Controller {
			...
			if(isset($header_menu->row()->added_pages)){
				if(!empty($header_menu->row()->added_pages)){
					...
					foreach($header_menu->row()->added_pages as $header_page){
						if($header_page!=''){
							$detail = $this->cms_model->get_all_details(CMS,array('_id'=>new MongoId($header_page)));
		...
		if($footer_menu->num_rows()>0){
			...
			if(isset($footer_menu->row()->added_pages)){
				if(!empty($footer_menu->row()->added_pages)){
					$i=0;
					#echo '<pre>'; print_r($footer_menu->row()->added_pages); die;
					foreach($footer_menu->row()->added_pages as $footer_page){ 
						if($footer_page!=''){
							$detail = $this->cms_model->get_all_details(CMS,array('_id'=>new MongoId($footer_page)));
    ...
    /**
     * 
     * This function is used for share referral code
     * 
     * */
    public function share() {
        $user_id = $this->uri->segment(2);
        $this->data['rider_info'] = $this->user_model->get_all_details(USERS, array('_id' => new \MongoId($user_id)));
    ...
    /**
     *
     * 	This functions checks the vehicle number is already exist or not
     *
     * */
	...
	function send_contact_mail(){ 
		...
		if($user_email != '' && $message != ''){
			...
			$this->setErrorMessage('error','Please fill the required fields');
		}
