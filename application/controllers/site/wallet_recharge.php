...
class Wallet_recharge extends MY_Controller {
    ...
    public function add_pay_wallet_payment_form() {
        ...
        $chckUser = $this->wallet_model->get_selected_fields(USERS, array('_id' => New \MongoId($user_id)), array('_id'));
    ...
    public function user_wal_PaymentCard() {
        ...
        if ($transaction_id != '' && $total_amount != '' && $user_id != '') {
            ...
            $userDetail = $this->wallet_model->get_selected_fields(USERS, array('_id' => new \MongoId($user_id)), array('user_name', 'email', 'phone_number'));
    ...
    public function pay_success() {
        ...
        $this->data['rider_info'] = $rider_info = $this->user_model->get_all_details(USERS, array('_id' => new \MongoId($user_id)));
        ...
        if ($payment_status == 'Completed') {
            ...
            if ($checkRecharge->num_rows() == 1) {
                if ($checkRecharge->row()->pay_status == 'Pending') {
                    ...
                    $this->wallet_model->simple_push(WALLET, array('user_id' => new \MongoId($user_id)), array('transactions' => $initialAmt));
