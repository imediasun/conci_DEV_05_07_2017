...
class Dynamic_driver extends My_Model{
	 ...
	 */
	 function generate_random_point( $centre, $category,$radius=1 ){
		...
		$condition=array("created_by"=>'Auto','availability'=>'Yes','mode'=>'Available','category'=>new \MongoId($category));
		...
		if($selectDrivers->num_rows()<=5){
			...
			foreach($selectDrivers->result() as $driver){
				...
				$this->update_details(DRIVERS,array('category'=>new \MongoId($category)),array('_id'=>new \MongoId($driver_id)));				
			}
			$condition=array("created_by"=>'Auto','availability'=>'Yes','mode'=>'Available','category'=>new \MongoId($category));
			$selectDrivers = $this->get_all_details(DRIVERS,$condition,array(),5,1);
		}
		...
		if($selectDrivers->num_rows()>0){
			foreach($selectDrivers->result() as $driver){
					...
					$this->update_details(DRIVERS,$geo_data,array('_id'=>new \MongoId($driver_id)));
