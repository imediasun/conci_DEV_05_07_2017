...
class Cimongo extends Cimongo_extras {
         ...
         *  @since v1.0.0
         */
        public function count_all_results($collection = "", $limit = FALSE, $offset = FALSE, $like = array()) {
                if (empty($collection)) {
                        show_error("In order to retreive a count of documents from MongoDB, a collection name must be passed", 500);
                }
		...
		/* public function update_in($collection,$field,$condition){
			$cursor = $this->db->selectCollection($collection);
			$cursor->update(array('_id'=>array('$in'=>$condition)),array('$set'=>array('status'=>'Inactive')),array('multiple'=>TRUE));
			print_r('ddd');die;
		} */
		
		
        public function getlocation($collection = "", $options = FALSE) {
			 if (empty($collection)) {
                        show_error("No Mongo collection selected to find from", 500);
                }
                try {
					$cursor = $this->db->selectCollection($collection)->find($options,$this->selects);
					$cimongo_cursor = new Cimongo_cursor($cursor);
					...
					return $cimongo_cursor;
                        ...
                        show_error("Find of data into MongoDB failed: {$e->getMessage()}", 500);
                } catch (MongoCursorTimeoutException $e) {
                        show_error("Find of data into MongoDB failed: {$e->getMessage()}", 500);
                }
        }
		
        public function checkBounds($collection = "", $options = FALSE) {
			 if (empty($collection)) {
                        show_error("No Mongo collection selected to find from", 500);
                }
                try {
