...
function bulk_logs_action(req,AdmEmail){ 
	...
	$( "#collect_admin_email" ).modal();
	$("#admin_bulkAction").val(req);
	$("#admin_bulkSecurityEmail").val(AdmEmail);
 ...
 function validate_admin_security_email(){
		var action = $("#admin_bulkAction").val();
		var AdmEmail = $("#admin_bulkSecurityEmail").val();
		var security_mail = $("#security_alertBox").val(); 
		if(security_mail == AdmEmail){
			$('#statusMode').val(action);
			$('#SubAdminEmail').val(AdmEmail);				
			$('#display_form').submit();
		} else {
			$("#security_alertBox").css('border','1px solid red');
		}
 }
