$(function () {
    ...
    $(".chzn-select1-deselect").chosen({allow_single_deselect: true});
    ...
    function available(date) {
        var from = $(this).attr("data-avail");
        var numbers = from.match(/\d+/g);
        var datea = new Date(numbers[2], numbers[0] - 1, numbers[1]);
        availableDates1 = datea.getDate() + "-" + (datea.getMonth() + 1) + "-" + datea.getFullYear();
...
//MIN Chart 
    ...
    jQuery(function ($) {
        ...
        $('.dialog-modal').click(function (e) {
            ...
            $('#' + contentId).modal();
            $('#' + contentId + "_form").validate();

            $(".datepicker_sd").datetimepicker({
                dateFormat: "yy-mm-dd",
                //changeYear: true,
                changeMonth: true,
                minDate: '-3m',
...
// color picker
    ...
    $("#addcategory_form,#editcategory_form").validate();
    $("#addattribute_form,#edituser_form").validate();
    $("#editpromo_form,#addpromo_form").validate();
    $("#editbrand_form,#addbrand_form").validate();
    $("#addEditvehicle_form").validate();
    $("#driver_form").validate();
    $("#addeditmodel_form").validate();
    $("#addeditcategory_form").validate();
    $("#addeditcategorytypes_form").validate();
    $("#addeditdriverbank_form").validate();
    $("#addeditlocation_form").validate();
    $("#addeditfare_form").validate();
    $("#addeditcountry_form").validate();
    $("#addeditcurrency_form").validate();
    $("#smsForm,#landing_page_form").validate();
    $("#app_settings_form").validate();
    $("#addEditcancellation_form").validate();
    $("#admin_cancelling_ride_form").validate();
    ...
    $("#addsubadmin_form").validate({
        ...
        messages: {
            email: {
                required: sub_admin_email_txt,
                email: valid_email_address_txt
            },
            admin_name: {
                required: admin_username_txt,
                minlength: username_length_txt
            },
            admin_password: {
                required: new_admin_password_txt,
                minlength: password_length_txt
            }
        }
    });
    ...
    $("#regitstraion_form").validate({
        ...
        messages: {
            name: firstname_txt,
            username: {
                required: username_txt,
                minlength: username_length_txt
            },
            password: {
                required: password_txt
            },
            new_password: {
                required: new_password_txt,
                minlength: password_length_txt
            },
            confirm_password: {
                required: retypr_password_txt,
                minlength: password_length_txt,
                equalTo: same_password_txt
            },
            email: valid_email_address_txt,
            agree: accept_policy_txt
        }
    });
...
$(function () {
    $('.data_tbl').dataTable({
        ...
        "iDisplayLength": 50,
        "oLanguage": {
            "sLengthMenu": "<span class='lenghtMenu'> _MENU_</span><span class='lengthLabel'>"+datatable_entries_per_page+":</span>",
            "sEmptyTable": datatable_no_data_available,
            "sSearch": datatable_search,
            "sZeroRecords": datatable_no_record_found,
            "oPaginate": {
				"sFirst" : pagination_first,
				"sPrevious" : pagination_previous,
				"sNext" : pagination_next,
				"sLast" : pagination_last,
			}
        },
...
//		                 "aaSorting": [[0, 'desc']],
    ...
    $('#driverListTbl').dataTable({
        "aoColumnDefs": [
            {"bSortable": false, "aTargets": [0, 7,11]}
        ],
//		                 "aaSorting": [[0, 'desc']],
        "sPaginationType": "full_numbers",
        "iDisplayLength": 50,
    ...
    $('#revenueListTblCustom').dataTable({
        "aoColumnDefs": [{"bSortable": false, "aTargets": []}],
        "aaSorting": [[8, 'desc']],
        "sPaginationType": "full_numbers",
        "iDisplayLength": 50,
...
$(function () {
    ...
    nCloneTd.innerHTML = '<img src="images/site/details_open.png">';
...
/* Custom Script for table */

/* Formating function for row details */
function fnFormatTripDetails(nTr) {
    var nTrId = $(nTr).attr('id');
    var sOut = '<table cellpadding="5" cellspacing="0" border="0" style="padding-left:50px;">';
    sOut += '<tr><td>'+admin_ride_pickup_location+' : </td><td>' + $("#" + nTrId + " em").attr('data-pickup') + '</td></tr>';
    sOut += '<tr><td>'+admin_ride_drop_location+' : </td><td>' + $("#" + nTrId + " em").attr('data-drop') + '</td></tr>';
    sOut += '<tr><td>'+admin_ride_payment_by+' : </td><td>' + $("#" + nTrId + " em").attr('data-paytype') + '</td></tr>';
    sOut += '</table>';
    return sOut;
}
/* Formating function for row details */
function chkOpen(nTr) {
    var nTrId = $(nTr).attr('id');
    var imgSrc = $("#" + nTrId + " .open_close").attr("src");
    var imgVal = "images/details_open.png";
    if (imgSrc == imgVal) {
        return false;
    } else {
        return true;
...
$(function () {
    ...
    var nCloneTh = document.createElement('th');
    var nCloneTd = document.createElement('td');
    nCloneTd.innerHTML = '<img class="open_close" src="images/details_open.png">';
    nCloneTd.className = "center";

    $('.tbl-open-close thead tr').each(function () {
        this.insertBefore(nCloneTh, this.childNodes[0]);
    });

    $('.tbl-open-close tbody tr').each(function () {
        this.insertBefore(nCloneTd.cloneNode(true), this.childNodes[0]);
    });
    ...
    var oTable = $('.tbl-open-close');

    /* Add event listener for opening and closing details
     * Note that the indicator for showing which row is open is not controlled by DataTables,
     * rather it is done here
     */
    $('.tbl-open-close tbody td img.open_close').live('click', function () {
        var nTr = $(this).parents('tr')[0];
        var cO = chkOpen(nTr);
        if (cO) {
            this.src = "images/details_open.png";
            ...
            this.src = "images/details_close.png";
            appendDetails(nTr, fnFormatTripDetails(nTr), 'details');
        }
    });
});

function appendDetails(nTr, mHtml, sClass) {
    var nNewRow = document.createElement("tr");
    var nNewCell = document.createElement("td");
    nNewRow.appendChild(nNewCell);
    nNewCell.className = sClass;
    count = $(nTr).children("td").index() + 1;
    nNewCell.colSpan = count;
